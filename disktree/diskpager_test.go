...
func TestDiskPager(t *testing.T) {
	...
	// 创建RedoLog实例
	redoLog, err := NewRedoLog(logFilename)
	if err != nil {
		t.Fatalf("Failed to create RedoLog: %v", err)
	}
	defer redoLog.Close()

	// 创建DiskPager实例
	pager, err := NewDiskPager(filename, pageSize, cacheSize, redoLog)
	if err != nil {
	...
	t.Run("Multiple Pages", func(t *testing.T) {
		...
		err = pager.WritePage(pageNum2, testData2, 01)
		if err != nil {
	...
	t.Run("Error Cases", func(t *testing.T) {
		...
		err = pager.WritePage(0, wrongSizeData, 01)
		if err == nil {
...
func seeDetail(filename string, pageSize int) {
	logger.Debug(":::Start to show detail of the file:")
	// 读取 db 文件，看是否新建页面成功？
	...
	f.Read(pageData)
	fmt.Printf("%v\n", pageData)
	fmt.Printf("内容(十六进制)：%x\n", pageData)

	logger.Debug(":::End of show detail of the file")
}
